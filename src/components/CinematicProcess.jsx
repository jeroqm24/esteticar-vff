...
export default function CinematicProcess() {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([entry]) => window.dispatchEvent(new CustomEvent("ec:dark", { detail: entry.isIntersecting })),
      { threshold: 0.01 }
    );
    obs.observe(el);
  ...
  useMotionValueEvent(scrollYProgress, "change", (p) => {
    const v = videoDesktopRef.current;
    ...
    <div ref={containerRef} data-dark-nav style={{ height: "520vh", position: "relative" }}>
      <div style={{ position: "sticky", top: 0, height: "100dvh", overflow: "hidden", background: "#040404" }}>
        ...
        <motion.div style={{ opacity: badgeOpacity }}
          className="absolute bottom-14 sm:bottom-16 lg:bottom-20 inset-x-0 z-20 flex justify-center pointer-events-none">
